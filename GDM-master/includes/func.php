...
function addDm(){
    ...
    $imagerie   =   e($_POST['imagerie']);
    $biologie   =   e($_POST['biologie']);
    $fonResp   =   e($_POST['fonResp']);
    $allergologie   =   e($_POST['allergologie']);
    $bacteriologie   =   e($_POST['bacteriologie']);
    $autres   =   e($_POST['autres']);
    $observation   =   e($_POST['observation']);
    if (empty($prenom) OR empty($nom) OR empty($sexe)) {
        array_push($errors, "insérer des données");
    }
    if (count($errors) == 0){
        $query = "INSERT INTO `dm` (`situation_famille`, `division`, `direction`, `unite`, `service`, `activite`, `atelier`, `maladie_gen`, `maladie_prof`, `maladie_carc`, `interventions`, `affectations`, `intoxications`, `accidents`, `collateraux`, `conjoints`, `imagerie`, `biologie`, `fonResp`, `allergologie`, `bacteriologie`, `autres`, `observation`) 
                            VALUES ('$situation', '$division', '$direction', '$unite', '$service', '$activite', '$atelier', '$maladie_gen', '$maladie_prof', '$maladie_carc', '$interventions', '$affectations', '$intoxications', '$accidents', '$collateraux', '$conjoints', '$imagerie', '$biologie', '$fonResp', '$allergologie', '$bacteriologie', '$autres', '$observation');";
